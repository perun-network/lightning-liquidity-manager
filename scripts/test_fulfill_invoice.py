#!/usr/bin/env python3
"""Test fulfill invoice transaction - parameterized validator version"""
...
@dataclass
class FulfillInvoiceRedeemer(PlutusData):
    """FulfillInvoice redeemer (Action::FulfillInvoice)"""
    CONSTR_ID = 3
    invoice: Invoice
...
def test_fulfill_invoice(contract_tx_id: str, invoice_id: int):
    """Test fulfill invoice operation"""
    print("=" * 70)
    print("Lightning Liquidity Manager - Fulfill Invoice Test")
    print("=" * 70)
    ...
    try:
        ...
        for raw_inv in current_state.invoices:
            # raw_inv is PlutusData, we need to convert it to Invoice
            if isinstance(raw_inv, Invoice):
                invoices_list.append(raw_inv)
            else:
                # It's raw PlutusData, deserialize it
                try:
        ...
        print(f"  - Owner PKH: {target_invoice.owner.hex()[:16]}...")

        # Calculate new state
        print(f"\n[4/6] Calculating new state...")

        # Remove fulfilled invoice from list - keep raw invoices except the target
        new_invoices_raw = []
        for raw_inv in current_state.invoices:
        ...
        new_state = State(
            total_liquidity=current_state.total_liquidity - target_invoice.amount,
        ...
        print(
            f"  New Liquidity:    {new_state.total_liquidity:,} (-{target_invoice.amount:,})")
        print(
            f"  New Reserved:     {new_state.reserved:,} (-{target_invoice.amount:,})")

        # Create redeemer
        redeemer = Redeemer(FulfillInvoiceRedeemer(invoice=target_invoice))
        ...
        new_cbtc = current_cbtc - target_invoice.amount

        # Prepare output value
        asset_name = AssetName(cbtc_asset_name)
        my_asset = Asset({asset_name: new_cbtc})
        multi_asset = MultiAsset(
            {ScriptHash.from_primitive(cbtc_policy): my_asset})

        # Build owner output
        owner_vkh = VerificationKeyHash(target_invoice.owner)
        owner_address = Address(
            payment_part=owner_vkh,
            network=Network.TESTNET,
        )

        owner_asset = Asset({asset_name: target_invoice.amount})
        owner_multi_asset = MultiAsset(
            {ScriptHash.from_primitive(cbtc_policy): owner_asset})
        ...
        # Script output with updated state
        builder.add_output(
            TransactionOutput(
        ...
        # Owner output with cBTC
        builder.add_output(
            TransactionOutput(
                address=owner_address,
                # 2 ADA + cBTC
                amount=Value(coin=2_000_000, multi_asset=owner_multi_asset),
            )
        )
        ...
        # Update log
        update_invoice_log(invoice_id, tx_hash, "fulfilled")

        print("\n" + "=" * 70)
        print("✓ INVOICE FULFILLED SUCCESSFULLY")
        print("=" * 70)
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        print("Usage: uv run scripts/test_fulfill_invoice.py <contract_tx_id> <invoice_id>")
        print("\nExample:")
        print("  uv run scripts/test_fulfill_invoice.py abc123...def 1")
        sys.exit(1)

    contract_tx = sys.argv[1]
    inv_id = int(sys.argv[2])

    sys.exit(test_fulfill_invoice(contract_tx, inv_id))
